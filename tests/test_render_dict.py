"""Tests for ``server.services.mykey_codec.render_value`` / ``render_dict``.
...
from server.services.mykey_codec import render_dict, render_value  # noqa: E402
...
class RenderListInlineVsBlockTests(unittest.TestCase):
    def test_short_list_inline(self):
        self.assertEqual(render_value([1, 2, 3]), "[1, 2, 3]")

    def test_long_list_breaks(self):
        items = [f"item-{i}" for i in range(20)]
        out = render_value(items)
        self.assertIn("\n", out)
...
class RenderDictStyleTests(unittest.TestCase):
    ...
    def test_continuation_indent_relative_to_level_not_caller_column(self):
        ...
        """
        out = render_dict({"name": "x", "llm_nos": [1, 2, 3, 4]})
