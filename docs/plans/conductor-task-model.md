...
> **状态标记（2026-09-20 复核）：🟡 部分完成 —— H1–H3 ✅、G1+G2 ✅、G3 ✅、G4 待复核。**
...
>   `extra_sys_prompts=[SUPERVISOR_PROTOCOL]`。
> - **G3**（新任务边界换归档 + 清上下文）✅ 最终采用与官方桌面端一致的**重建 agent**方案：同一
...
- 日期：2026-09-15
- 状态：GA-Hub 侧 H1–H3 **已实现**；GA 侧 **G1+G2 已完成（GA `04f0d9d`，09-18）、G3 已完成（2026-09-20）、G4 待复核**
...
| 4 | 一个任务一个归档：同 request 复用 supervisor；新 request 安全停止旧线程后重建 `GenericAgent`（方案 B，与官方桌面端一致） |
...
- G3 `conductor_core.py`：新 request 边界安全停止并 join 旧 supervisor，再由 `agent_factory` 重建 `GenericAgent`；同 request 追加复用当前实例。Hub 强制 F1 关闭，GA 保留显式 opt-in 兼容路径。
