...
> **状态标记（2026-09-18 复核）：🟡 代码已实施并已进包，§8.5 实机验收仍未完成。**
> - 三个长驻子进程已全部走 `server/services/child_job`（`f6f5732`），`cmd.exe` 中介启动于 `20ad092` 补齐。
> - **缺的是验收，不是实现**：§8.5「真实冻结 sidecar → 引擎能否活下来」需在**含 `20ad092` 的新包**上跑一次。
> - ✅ 该前提已满足：桌面端于 **2026-09-18 14:43/14:54 重建**，`ga-hub-sidecar.exe`（14:43:01）与
>   `ga-hub-desktop.exe`（14:54:23）均已含 `20ad092`。可直接开始 §8.5 验收。
>   （该项已按用户决定移出 `TODO_REMAINING.md`，此处仅保留事实。）

- 日期：2026-09-15
- 状态：**代码已实施**（见 §8 实施记录；实现与本文档的差异以 §8 为准）；
  **§8.5 实机验收未做**（2026-09-18 复核）
