...
export function LiveChat() {
  ...
  const currentTitle = useChatStore((s) => s.currentTitle)
  const setCurrentTitle = useChatStore((s) => s.setCurrentTitle)
  ...
  useEffect(() => {
    if (restoreState?.messages?.length) {
      ...
      clearLocal()
      setCurrentTitle(restoreState.restoredTitle || '')
  ...
  useEffect(() => {
    if (agentStatus) setCurrentTitle(agentStatus.current_title || '')
  }, [agentStatus, setCurrentTitle])

  useEffect(() => {
    const flushArchive = () => {
      if (typeof navigator === 'undefined' || msgs.length === 0 || streaming) return
      if (archiveSentRef.current) return
      archiveSentRef.current = true
      const payload = JSON.stringify({})
      try {
        navigator.sendBeacon('/api/agent/archive', new Blob([payload], { type: 'application/json' }))
      } catch {
        void fetch('/api/agent/archive', {
          method: 'POST',
    ...
    const onPageHide = () => flushArchive()
    const onBeforeUnload = () => flushArchive()
  ...
  const saveTitle = async () => {
    await api.agentSetTitle(currentTitle)
    qc.invalidateQueries({ queryKey: ['agent-status'] })
  }

  const autoTitle = async () => {
    const summarizeQuestion = (s: string) => s.replace(/\s+/g, ' ').trim().slice(0, 500)
    const summarizeAnswer = (s: string) => {
      const text = s.trim()
      const tag = text.match(/<summary>([\s\S]*?)<\/summary>/i)
      if (tag?.[1]?.trim()) return tag[1].replace(/\s+/g, ' ').trim().slice(0, 500)
      // Fallback: still send a compact answer summary, not the whole answer.
      return text
        .replace(/```[\s\S]*?```/g, ' ')
        .replace(/\[[^\]]{1,80}\]\([^)]{1,240}\)/g, ' ')
        .replace(/[#*_`>|-]+/g, ' ')
        .replace(/\s+/g, ' ')
        .trim()
        .slice(0, 500)
    ...
    const byStream = new Map<string, { q?: string; a?: string }>()
    const looseUsers: string[] = []
    const looseAssistants: string[] = []

    for (const m of msgs) {
      const content = m.content.trim()
      if (!content || m.role === 'system') continue
      if (m.streamId) {
        const pair = byStream.get(m.streamId) ?? {}
        if (m.role === 'user') pair.q = summarizeQuestion(content)
        if (m.role === 'assistant') pair.a = summarizeAnswer(content)
        byStream.set(m.streamId, pair)
      } else if (m.role === 'user') {
        looseUsers.push(summarizeQuestion(content))
      } else if (m.role === 'assistant') {
        looseAssistants.push(summarizeAnswer(content))
      }
    }

    for (const pair of byStream.values()) {
      if (!pair.q && !pair.a) continue
      summaries.push(`Q: ${pair.q ?? ''}\nA摘要: ${pair.a ?? ''}`)
    }
    const n = Math.max(looseUsers.length, looseAssistants.length)
    for (let i = 0; i < n; i += 1) {
      const q = looseUsers[i] ?? ''
      const a = looseAssistants[i] ?? ''
      if (q || a) summaries.push(`Q: ${q}\nA摘要: ${a}`)
    }

    const payload = summaries.filter(Boolean).slice(-8)
    if (payload.length === 0) {
      pushSystem('_没有可用于生成标题的多轮问答摘要_')
      return
    }

    setAutoTitling(true)
    try {
      const r = await api.agentAutoTitle({ summaries: payload })
      if (!r.ok) throw new Error(r.error || 'LLM未返回标题')
      setCurrentTitle(r.title || '')
      qc.invalidateQueries({ queryKey: ['agent-status'] })
      pushSystem(`_AI标题已更新：${r.title || '新对话'}_`)
    } catch (e) {
      pushSystem(`_AI标题生成失败：${e instanceof Error ? e.message : String(e)}_`)
    } finally {
      ...
      actions={
        <div className="flex items-center gap-2 text-sm">
          <input
            ...
            placeholder="当前对话标题"
            className="w-56 bg-bg-card border border-line rounded-lg px-3 py-1.5 text-sm outline-none focus:border-accent"
            title="编辑当前对话标题"
          />
          <button
            type="button"
            onClick={() => void autoTitle()}
            disabled={autoTitling || streaming || msgs.length === 0}
            className="px-2.5 py-1.5 rounded-lg border border-line bg-bg-card text-xs hover:border-accent disabled:opacity-50 disabled:cursor-not-allowed"
            title="用当前 LLM 为这段对话自动生成标题"
          >
            {autoTitling ? '生成中…' : 'AI标题'}
          </button>
