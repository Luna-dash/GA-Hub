...
function SummaryCard({ label, value, hint, tone }: { label: string; value: number; hint: string; tone: string }) {
  return <div className="rounded-xl border border-line bg-bg-card px-4 py-4 shadow-sm">
...
function Composition({ title, data }: { title: string; data: TokenTotals }) {
  ...
  return <section className="rounded-xl border border-line bg-bg-card p-4 shadow-sm">
...
function UsageTrend({ rows, range, setRange, currentWeek, timestamp }: { rows: TokenDayStats[]; range: DailyUsageRange; setRange: (range: DailyUsageRange) => void; currentWeek: TokenStatsResponse['current_week']; timestamp: number }) {
  ...
  return <section className="rounded-xl border border-line bg-bg-card p-4 shadow-sm">
    <div className="flex flex-wrap items-center justify-between gap-3">
      ...
      <div className="flex rounded-lg bg-bg-soft p-0.5">{([{ value: 'week', label: '本周' }, { value: '30d', label: '近 30 日' }] as const).map(option => <button key={option.value} type="button" onClick={() => setRange(option.value)} className={`rounded-md px-3 py-1 text-xs transition ${range === option.value ? 'bg-bg-card font-medium text-slate-700 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}>{option.label}</button>)}</div>
    </div>
...
function SessionUsage({ rows }: { rows: TokenThreadStats[] }) {
  ...
  return <section className="overflow-hidden rounded-xl border border-line bg-bg-card shadow-sm">
    <div className="border-b border-line px-4 py-3"><h2 className="text-sm font-semibold text-slate-700">高用量会话</h2><p className="mt-0.5 text-[11px] text-slate-400">按 Token 总量排名 · Top {Math.min(rows.length, 6)}</p></div>
    {leading.length ? <div className="grid gap-3 p-4 lg:grid-cols-2">{leading.map((row) => {
      ...
      return <article key={row.thread} className="min-w-0 rounded-lg border border-line/80 bg-bg-soft/50 p-3">
        <div className="truncate text-xs font-medium text-ink" title={sessionTitle(row)}>{sessionTitle(row)}</div>
        <div className="mt-3 grid grid-cols-3 gap-x-2 gap-y-3 sm:grid-cols-6">{metrics.map(([label, value]) => <div key={label} className="min-w-0"><div className="text-[10px] text-slate-400">{label}</div><div className={`mt-0.5 truncate font-mono text-xs ${label === '合计' ? 'font-semibold text-slate-700' : 'text-ink-faint'}`} title={label === '合计' ? exact(row.total) : value}>{value}</div></div>)}</div>
      </article>
