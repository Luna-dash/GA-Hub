...
export const api = {
  ...
  agentSetTitle: (title: string) => http<{ ok: boolean; title: string }>('PATCH', '/api/agent/title', { title }),
  agentAutoTitle: (payload: { summaries: string[] }) =>
    http<{ ok: boolean; title: string; error?: string }>('POST', '/api/agent/title/auto', payload),
  agentAbort: () => http<{ ok: boolean }>('POST', '/api/agent/abort'),
  agentNew: () => http<{ ok: boolean; message: string }>('POST', '/api/agent/new'),
  agentArchive: () => http<{ ok: boolean; saved: boolean }>('POST', '/api/agent/archive'),
