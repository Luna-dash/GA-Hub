...
- **Browser debug bridge:** the shell also binds a fixed loopback port (default `8765`; override with `GA_HUB_BRIDGE_PORT`, `0` disables) and forwards raw TCP to the owning sidecar's random port, so `http://127.0.0.1:8765/` reaches the live desktop backend from a regular browser for debugging (same-origin SPA, HTTP and WebSocket). The bridge is additive: if the port is already taken (for example by a manually launched `python -m server.run`), it is skipped with a diagnostic and the occupant is left untouched. It targets the sidecar port captured at startup, so it keeps working across `restart_backend`, and it disappears with the shell.
