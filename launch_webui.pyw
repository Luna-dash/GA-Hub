...
WINDOWS_APP_ID = "GenericAgent.Admin"
WINDOWS_ICON = SCRIPT_DIR / "assets" / "genericagent_admin.ico"
...
def _setup_windows_app_id() -> None:
    ...
    try:
        ...
        print(f"[Launch] SetCurrentProcessExplicitAppUserModelID failed: {e}", file=sys.stderr)
...
def main() -> int:
    ...
    # Do not start the Windows tray integration here: its close-button hook
    # hides the window and keeps the backend Python process alive. Product
    # expectation for this app is that closing the main window fully exits.
    tray = None
    start_kwargs = {}
    if os.name == "nt" and WINDOWS_ICON.exists():
        start_kwargs["icon"] = str(WINDOWS_ICON)
    try:
